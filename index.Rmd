...
First ssh to pelle cluster in UPPMAX

```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
ssh -AX yourusername@pelle.uppmax.uu.se
```

Start an interactive uppmax session for two hours

```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
interactive -A uppmax2026-1-94 -t 02:00:00
```

Make a directory in your user space (only if you have not done before)

```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
mkdir -p /crex/proj/uppmax2026-1-94/nobackup/$USER/nextflow_lab1
```

Navigate to the folder you have created

```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
cd /crex/proj/uppmax2026-1-94/nobackup/$USER/nextflow_lab1
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
// single file
myFileChannel = Channel.fromPath( '/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/Blank10.mzML' ) // #1
myFileChannel.view()
// multiple files
myFileChannel = Channel.fromPath( '/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.*' ) // #2
myFileChannel.view()
// recursive multiple files
myFileChannel = Channel.fromPath( '/crex/proj/uppmax2026-1-94/metabolomics/**.*' ) // #3
myFileChannel.view()
...
The files are in the path "*/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*".
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
mzMLFiles = Channel.fromPath( '/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML' ) // #1
...
1. Creates a file channel from all `mzML` files in `/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML'`.
...
In the above example the process, when executed, it will create a file channel from `/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML`. The process will then get this channel as an input and creates an output channel `outputChannel` (named in the workflow) where each file extension has been changed to `featureXML`.
...
params {
   ...
   mzMLFilesInput = '/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML' // #1
  ppm_input=10 // #2
...
process {
executor = 'slurm' // #1
  clusterOptions = { "-A $params.project" } // #2
}
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_13.nf -c nextflow_13.config --project "uppmax2026-1-94"
```
...
Please be aware that if you use Nextflow to use a Docker image and convert to singularity image. This process will take time. In order to save you time we have already done the conversion. To use what we have built, first run the following command:
...
```{bash, attr.source='.numberLines',eval=FALSE}
nextflow main_15.nf -c nextflow_15.config --project "uppmax2026-1-94"
```
...
Save the above in a file as `nextflow.config` in the directory where the `main_15.nf` is kept.
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_15.nf -profile standard
```

How would you run it if you wanted to run on UPPMAX?

```{bash, attr.source='.numberLines',eval=FALSE}
nextflow main_15.nf -profile uppmax_singularity --project "uppmax2026-1-94"
```
...
In this example, the process `featureFinder` sends output to an output channel (featureFinder_out) (#1) which is then sent to `someotherprocess` (#2) to do further processing. However, `someotherprocess` now has all the data at once.
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_18.nf --inputmzml "/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML"
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main_19.nf --inputmzml "/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML" --qc 100
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow run main_19.nf --inputmzml "/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML" --qc 100
```
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow run main_19.nf -resume --inputmzml "/crex/proj/uppmax2026-1-94/metabolomics/mzMLData/*.mzML" --qc 100
```
...
**IMPORTANT: Sometimes the Uppmax file system might cause some of the processes to fail, if this happens, rerun the workflow with -resume option. This might actually pass the step that was previously shown as failed!**
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
nextflow main.nf -profile uppmax --project "uppmax2026-1-94" -resume
```
...
You can use jobinfo by logging in to uppmax via ssh using another terminal.
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
squeue -u $USER
```
...
*   Exercise one: There are some hardcoded parameters part of the pipeline:
    *   In process process_masstrace_detection_pos_xcms we have the following parameters:
...
**Can you change the config file so the value of these parameters is set there and fetched inside the main.nf file?**
**Also, show us how the user can pass these parameters while running nextflow command? Show the command**
...
## Part 2 (Build an OpenMS pipeline)
**For part 2, you will need to send us the main.nf file.**  
...
Let’s name the first process `process_masstrace_detection_pos_OpenMS`.
   ...
   1. This process has two inputs:  
      a. The first input is from the mzML files  
      b. The second one is from the **feature finder** parameter file. Please remember that The second input should be a type of input that repeats for **each** of the mzML file emitted by the first channel  
...
The second process is called `process_masstrace_alignment_pos_OpenMS`
   ...
   4. Similar to the previous process it will get two inputs:  
      a. The first one is the output of `process_masstrace_detection_pos_OpenMS` - the previous process  
      b. The second one is from the alignment parameter file. Similar to the previous one this input is also repeating! Remember using **each**?  
   5. The output for this process has the same `baseName` as input and it has `.featureXML` extension. However, these files are supposed to be kept in a different   output folder (let’s call it `out`). The output channel must be named `LinkerProcess`  
   6. The command needed is a bit different from the previous one:  

Unlike the FeatureFinder step, the alignment step should run once on all featureXML files together. Therefore, collect the featureXML files before passing them to this process.

Unlike the FeatureFinder step, the alignment step should run once on all featureXML files at **the same time** (look at collect!). Therefore, collect the featureXML files from the previous process before passing them to this process. The alignment also outputs the same number of files. For the command, the inputs must be separated by space (remember the join operation in the XCMS pipeline?). The output is the same as input however you need to put the output in a different folder (`out`) within the process! Think about a simple bash script. You create a folder and join the files in a way that they are written in a separate folder! 
  
Inside the script block:  
- create an output directory called out.  
- join the input featureXML filenames with spaces for the -in argument.  
- create matching output filenames inside out/ for the -out argument.  
...
```{bash, attr.source='.numberLines',eval=FALSE, class.source="nohide"}
mkdir out
MapAlignerPoseClustering -in x.featureXML y.featureXML z.featureXML -out out/x.featureXML out/y.featureXML out/z.featureXML -ini setting.ini
```
...
The third process is called `process_masstrace_linker_pos_OpenMS` and is used to link (merge) multiple files into a single file
   ...
   7. It takes two inputs:  
      a. The output from the alignment - the previous process  
      b. The parameter file (`featureLinker.ini`)  
...
<!--
### Part X - PCA (optional to test you skills - **DOES NOT COUNT TOWARDS GRADING**)
...
**If you encounter issues with Uppmax being way too slow or does not do anything, please let us know!**
