...
ssh -AX youusername@pelle.uppmax.uu.se
```
...
*For part 2, you will need to send us the main.nf and the nextflow.config.*
...
Tip: For the second task, show how to run the pipeline by providing input via the command line. eg: "*nextflow main.nf -profile uppmax --project uppmax2026-1-94 --YourParametersHere.*"  
  
Help me: [params scope](https://www.nextflow.io/docs/latest/config.html#scope-params)


## Part 2
*For part 2, you will need to send us the main.nf and the nextflow.config files*  
...
I. Let’s name the first process `process_masstrace_detection_pos_OpenMS`
...
II. The second process is called `process_masstrace_alignment_pos_OpenMS`
...
III. The third process is called `process_masstrace_linker_pos_OpenMS` and is used to link (merge) multiple files into a single file
...
IV. Finally the last step which takes the xml formatted output from the previous step and convert it into a csv file. Let’s call this process `process_masstrace_exporter_pos_OpenMS`
...
### Part 2.2 PCA (optional to test you skills - **DOES NOT COUNT TOWARDS GRADING**)
